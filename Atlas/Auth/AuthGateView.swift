    ...
    var body: some View {
        Group {
            if authStore.isAuthenticated == false {
                ...
                ProfileLoadingView()
            } else if authStore.isProfileComplete == false {
                CreateUsernameView()
            } else {
                ContentView()
            }
...
private struct CreateUsernameView: View {
    @EnvironmentObject private var authStore: AuthStore
    ...
    var body: some View {
        ZStack {
            ...
            VStack(alignment: .leading, spacing: AppStyle.sectionSpacing) {
                ...
                .disabled(isWorking || isUsernameValid == false)
    ...
    private func normalizedInput() -> String {
        usernameInput.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    }
    ...
    private func submit() async {
        ...
        guard isUsernameValid else {
            status = .error("Usernames are 3–20 chars: a–z, 0–9, underscore.")
            return
        }
        isWorking = true
        status = nil
