...
- Cloud sync coordinator/state: `CloudSyncCoordinator` runs per-session sync + backfill using `CloudSyncService`, `AuthStore`, and `HistoryStore` ended sessions. `CloudSyncState` (UserDefaults) tracks `lastSyncedEndedAt` and recent session IDs. Wired in `AtlasApp` and attached to `HistoryStore`.
